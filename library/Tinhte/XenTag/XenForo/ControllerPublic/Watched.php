...
class Tinhte_XenTag_XenForo_ControllerPublic_Watched extends XFCP_Tinhte_XenTag_XenForo_ControllerPublic_Watched
{
    ...
    {
        $tagsWatched = $this->_Tinhte_XenTag_getTagWatchModel()->getUserTagWatchByUserId(XenForo_Visitor::getUserId());

        $viewParams = array('tags' => $tagsWatched);

        return $this->responseView('Tinhte_XenTag_ViewPublic_Watched_Tags', 'tinhte_xentag_watched_tags', $viewParams);
    }
    ...
    public function actionTagsUpdate()
    {
        ...
        if (!empty($input['tag_ids'])) {
            $tagWatches = $this->_Tinhte_XenTag_getTagWatchModel()->getUserTagWatchByUserIdAndTagIds(
        ...
        return $this->responseRedirect(
            ...
            $this->getDynamicRedirect(XenForo_Link::buildPublicLink('watched/tags'))
        );
    }
    ...
    protected function _takeEmailAction(array $user, $action, $type, $id)
    {
        if ($type == '' || $type == 'tag') {
            if ($id) {
                if (empty($action)) {
                    // oops, delete it
                    $this->_Tinhte_XenTag_getTagWatchModel()->setTagWatchState($user['user_id'], $id);
                } else {
                    $this->_Tinhte_XenTag_getTagWatchModel()->setTagWatchState($user['user_id'], $id, null, 0);
                }

            } else {
                $this->_Tinhte_XenTag_getTagWatchModel()->setTagWatchStateForAll($user['user_id'], $action);
            }
        }
    }
